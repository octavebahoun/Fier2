...
```
GET    /news                  → liste paginée (filtres: theme, domain, status, date)
GET    /news/:id              → détail article
POST   /news                  → créer un article (statut par défaut: PENDING)  [auth]
PUT    /news/:id              → modifier               [auth]
DELETE /news/:id              → supprimer              [auth]
PATCH  /news/:id/approve      → approuver un article   [auth, admin]
```
...
### 💼 MODULE : OPPORTUNITÉS (`/opportunities`)

```
GET    /opportunities         → liste des opportunités (filtres: type, domain, status)
GET    /opportunities/:id     → détail d'une opportunité
POST   /opportunities         → créer une opportunité  [auth]
PUT    /opportunities/:id     → modifier               [auth]
DELETE /opportunities/:id     → supprimer              [auth]
```
...
|--------|-----------|-------------|
| Actualités | 6 | POST/PUT/DELETE + approve |
...
| Stats | 1 | Non |
| Opportunités | 5 | POST/PUT/DELETE |
| **TOTAL** | **~61** | |